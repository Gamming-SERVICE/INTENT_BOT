# ══════════════════════════════════════════════════════════════════════════════
#                   Intent™ BOT v3.0 — Economy Cog (Hardened)
# ══════════════════════════════════════════════════════════════════════════════
...
_ROB_WIN_CHANCE    = 0.40   # 40 % success
_ROB_FINE_PERCENT  = 0.20   # 20 % of robber's wallet as fine on fail
_ROB_MAX_STEAL     = 5_000  # cap on steal amount
_ROB_MIN_VICTIM_BAL = 200   # victim must have at least this

# Per-user rob lock — prevents concurrent rob calls
_rob_locks: dict[tuple[int, int], asyncio.Lock] = {}
...
async def _add_balance(user_id: int, guild_id: int, amount: int) -> int:
    """Add (or subtract) balance atomically. Returns new balance. Never goes below 0."""
    await db.execute(
...
class Economy(commands.Cog, name="Economy"):
    """Virtual economy: balance, daily rewards, work, transfers, and robbery."""

    def __init__(self, bot: commands.Bot) -> None:
        self.bot = bot

    async def _gate(self, ctx: commands.Context) -> bool:
        """Return True if economy is enabled for this guild."""
    ...
    @commands.hybrid_command(name="daily", description="Claim your daily coins (24h cooldown)")
    @commands.guild_only()
    ...
    async def daily(self, ctx: commands.Context) -> None:
                ...
                if diff.total_seconds() < 86400:
                    ...
                    return await ctx.send(
                        embed=emb.warning(
                            ...
                            f"Next claim available in **{h}h {m}m**."
                        )
                    )
            except ValueError:
                pass
        ...
        new_balance = await _add_balance(ctx.author.id, ctx.guild.id, amount)
        await db.execute(
            "UPDATE users SET daily_at = ? WHERE user_id = ? AND guild_id = ?",
            (now.isoformat(), ctx.author.id, ctx.guild.id),
        )
        await db.record_transaction(ctx.guild.id, ctx.author.id, "daily", amount, new_balance)
        embed = emb.build(
            title="🎁 Daily Reward Claimed!",
            description=f"You received **{gs.currency_symbol} {amount:,}**!\nNew wallet: **{gs.currency_symbol} {new_balance:,}**",
            color=discord.Color.gold(),
            author=ctx.author,
        )
        await ctx.send(embed=embed)
    ...
    @commands.hybrid_command(name="work", description="Work for coins (1 hour cooldown)")
    @commands.guild_only()
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def work(self, ctx: commands.Context) -> None:
        ...
        await db.record_transaction(ctx.guild.id, ctx.author.id, "work", amount, new_balance)
        response = random.choice(WORK_RESPONSES).format(
            amount=f"{amount:,}", symbol=gs.currency_symbol
        )
    ...
    async def pay(self, ctx: commands.Context, member: discord.Member, amount: int) -> None:
        ...
        if amount <= 0:
            return await ctx.send(embed=emb.error("Amount must be a positive number."))
        if amount > 10_000_000:
            return await ctx.send(embed=emb.error("Maximum single transfer is 10,000,000."))
        ...
        if data["balance"] < amount:
            return await ctx.send(
                embed=emb.error(
                    f"Insufficient funds.\nWallet: {gs.currency_symbol} {data['balance']:,} | "
                    f"Needed: {gs.currency_symbol} {amount:,}"
                )
            )
        ...
        await db.record_transaction(ctx.guild.id, ctx.author.id, "pay_out", -amount,
                                     data["balance"] - amount)
        await db.record_transaction(ctx.guild.id, member.id, "pay_in", amount, 0)
    ...
    @commands.hybrid_command(name="rob", description="Attempt to rob another member (40% chance, 5 min cooldown)")
    ...
    async def rob(self, ctx: commands.Context, member: discord.Member) -> None:
        ...
        # Prevent concurrent rob attempts on same user
        ...
        async with lock:
            ...
            if victim["balance"] < _ROB_MIN_VICTIM_BAL:
                return await ctx.send(
                    embed=emb.warning(
                        ...
                        f"(minimum {gs.currency_symbol} {_ROB_MIN_VICTIM_BAL:,})."
                    )
                )
            ...
            if random.random() < _ROB_WIN_CHANCE:
                ...
                log.info("Rob success: %s stole %d from %s in guild %s",
                         ctx.author.id, stolen, member.id, ctx.guild.id)
            else:
                ...
                await ctx.send(
                    embed=emb.build(
                        ...
                        description=(
                            f"You failed to rob {member.mention} and were fined "
                            f"**{gs.currency_symbol} {fine:,}**!\n"
    ...
    async def deposit(self, ctx: commands.Context, amount: str = "all") -> None:
        ...
        if amount.lower() == "all":
            ...
            try:
                ...
                return await ctx.send(embed=emb.error("Invalid amount. Use a number or 'all'."))
    ...
    async def withdraw(self, ctx: commands.Context, amount: str = "all") -> None:
        ...
        if amt <= 0 or amt > data["bank"]:
            return await ctx.send(
                embed=emb.error(f"Invalid amount. Bank: {gs.currency_symbol} {data['bank']:,}")
            )
    ...
    @commands.hybrid_command(name="richlist", aliases=["leaderboard", "lb"], description="Top 10 wealthiest members")
    @commands.guild_only()
    ...
    async def addmoney(self, ctx: commands.Context, member: discord.Member, amount: int) -> None:
        ...
        gs = await GuildSettings.get(ctx.guild.id)
        await db.record_transaction(ctx.guild.id, member.id, "admin_add", amount, new_bal,
                                     {"by": ctx.author.id})
        await ctx.send(
            embed=emb.success(
                f"Added **{gs.currency_symbol} {amount:,}** to {member.mention}.\n"
                f"New wallet: {gs.currency_symbol} {new_bal:,}"
            )
        )
    ...
    async def removemoney(self, ctx: commands.Context, member: discord.Member, amount: int) -> None:
        ...
        new_bal = await _add_balance(member.id, ctx.guild.id, -amount)
        gs = await GuildSettings.get(ctx.guild.id)
        await db.record_transaction(ctx.guild.id, member.id, "admin_remove", -amount, new_bal,
                                     {"by": ctx.author.id})
        await ctx.send(
            embed=emb.success(
    ...
    @commands.hybrid_command(name="resetbalance", description="[Admin] Reset a member's economy to zero")
