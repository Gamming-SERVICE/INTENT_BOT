...
class Tickets(commands.Cog, name="Tickets"):
    ...
    async def list_tickets(self, ctx: commands.Context) -> None:
        rows = await db.fetchall(
            "SELECT channel_id, user_id, created_at FROM tickets WHERE guild_id = ? AND status = 'open' ORDER BY created_at DESC",
            (ctx.guild.id,),
        )
        if not rows:
        ...
        for r in rows[:20]:
            ...
            ch_mention = ch.mention if ch else f"*(deleted {r['channel_id']})*"
            lines.append(f"{ch_mention} — <@{r['user_id']}> — <t:{int(discord.utils.parse_time(r['created_at']).timestamp())}:R>")
        embed = emb.build(
    ...
    @commands.hybrid_command(name="closeticket", description="Force-close a ticket by channel")
    ...
    async def closeticket(self, ctx: commands.Context, channel: discord.TextChannel | None = None) -> None:
        ...
        if not row:
            return await ctx.send(embed=emb.error("That channel is not an open ticket."))
        await db.execute(
            "UPDATE tickets SET status = 'closed', closed_at = CURRENT_TIMESTAMP WHERE id = ?",
            (row["id"],),
        )
        await ctx.send(embed=emb.success(f"Ticket {target.mention} closed. Deleting in 5 seconds…"))
